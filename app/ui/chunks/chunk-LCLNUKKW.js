import{b as H}from"./chunk-VUYAJJE6.js";import{i as $}from"./chunk-NYPYZHXP.js";import{a as P,c as g,d as _}from"./chunk-XSKVCSHS.js";import{c as p,d as f,e as A,f as N,i as R,j as h}from"./chunk-RP7N3B43.js";import{e as n,h as w}from"./chunk-6R6CGC23.js";import{a as b,b as d,d as c,e as F,f as E}from"./chunk-OPL7LVHU.js";function oe(){n.advancedDraft=M(),u(),requestAnimationFrame(()=>b("advanced-route-form")?.scrollIntoView({behavior:"smooth",block:"start"}))}function le(){let e=f();if(e.length<2)throw new Error("Direct outside forwarding needs separate inbound and outbound gateways. Add or enable a second gateway first.");n.advancedDraft={id:"",name:"Direct outside forward",ingress_kind:"gateway",ingress_value:e[0].extension,enabled:!0,stages:[{id:`stage_${$(4)}`,name:"Forward immediately",ring_seconds:60,max_call_seconds:0,answer_mode:"first_answer",max_answered:1,targets:[{id:`target_${$(4)}`,kind:"external",value:"",trunk:e[1].extension,label:"Outside destination",enabled:!0}]}]},u(),requestAnimationFrame(()=>b("advanced-route-form")?.scrollIntoView({behavior:"smooth",block:"start"}))}async function ce(){let e={transfer_code:String(n.callFeatures?.transfer_code||"").trim(),conference_code:String(n.callFeatures?.conference_code||"").trim(),invite_listen_code:String(n.callFeatures?.invite_listen_code||"*86").trim(),invite_whisper_code:String(n.callFeatures?.invite_whisper_code||"*87").trim(),invite_barge_code:String(n.callFeatures?.invite_barge_code||"*88").trim(),enabled:n.callFeatures?.enabled!==!1},a=await _("api/call-features",{method:"PUT",body:JSON.stringify(e)});n.callFeatures=a,n.callFeaturesError="",g("Site phone codes saved and applied."),u()}async function ue(){let e=String(n.activeInvite.source_extension||"").trim(),a=String(n.activeInvite.target||"").trim();if(!e||!a)throw new Error("Choose the active participant and enter who should be invited.");let t=await _("api/active-call-invite",{method:"POST",body:JSON.stringify({source_extension:e,target:a,mode:n.activeInvite.mode||"barge",timeout_sec:30})});return g(`${n.activeInvite.mode==="barge"?"Participant":n.activeInvite.mode==="whisper"?"Coach":"Listener"} invitation sent to ${a}.`),t}function ve(e){let a=n.advancedRoutes.find(t=>String(t.id)===String(e));a&&(n.advancedDraft=structuredClone(a),!n.advancedDraft.enabled&&(n.advancedDraft.stages||[]).some(t=>t.answer_mode==="private_hub")&&(n.advancedDraft._disabled_for_private_hub=!0),u(),requestAnimationFrame(()=>b("advanced-route-form")?.scrollIntoView({behavior:"smooth",block:"start"})))}function pe(){n.advancedDraft=null,u()}function ge(){if(n.advancedDraft){if(n.advancedDraft.stages.length>=10){g("A route can contain at most 10 stages.");return}n.advancedDraft.stages.push({id:`stage_${$(4)}`,name:`Stage ${n.advancedDraft.stages.length+1}`,ring_seconds:20,max_call_seconds:0,answer_mode:"first_answer",max_answered:1,targets:[]}),u()}}function be(e){if(n.advancedDraft){if(n.advancedDraft.stages.length<=1){g("A route needs at least one stage.");return}n.advancedDraft.stages.splice(Number(e),1),u()}}function T(e){return e==="haos"?n.nodes[0]?.id||"":e==="gateway"?f()[0]?.extension||"":e==="sip"&&n.sip.map(p).filter(a=>a&&!h(a))[0]?.extension||""}function me(e){let a=n.advancedDraft?.stages?.[Number(e)];if(!a)return;if(a.targets.length>=20){g("A stage can contain at most 20 destinations.");return}let t=T("sip")?"sip":T("haos")?"haos":"gateway",s=a.targets.some(i=>i.role==="hub");a.targets.push({id:`target_${$(4)}`,kind:t,value:T(t),trunk:"",label:"",role:a.answer_mode==="private_hub"&&!s?"hub":"spoke",enabled:!0}),u()}function fe(e,a){let t=n.advancedDraft?.stages?.[Number(e)];t&&(t.targets.splice(Number(a),1),u())}function G(e){let a=structuredClone(e);return delete a._disabled_for_private_hub,a.name=String(a.name||"").trim(),a.ingress_value=String(a.ingress_value||"").trim(),a.stages=(a.stages||[]).map((t,s)=>({...t,id:t.id||`stage_${s+1}`,name:String(t.name||`Stage ${s+1}`).trim(),ring_seconds:Number(t.ring_seconds)||20,max_call_seconds:Math.max(0,Number(t.max_call_seconds)||0),max_answered:t.answer_mode==="first_answer"?1:Number(t.max_answered)||2,targets:(t.targets||[]).map((i,o)=>({...i,id:i.id||`target_${s+1}_${o+1}`,value:String(i.value||"").trim(),trunk:String(i.trunk||"").trim(),label:String(i.label||"").trim(),enabled:i.enabled!==!1}))})),a}function D(e){if(!String(e?.name||"").trim())return"Enter a plan name.";if(!["gateway","sip"].includes(String(e?.ingress_kind||"")))return"Choose whether calls enter from a gateway or SIP phone.";if(!String(e?.ingress_value||"").trim())return"Choose the exact incoming gateway or SIP phone.";if(!Array.isArray(e?.stages)||!e.stages.length)return"Add at least one routing stage.";let a=new Map,t=String(e?.ingress_kind||"").trim().toLowerCase(),s=String(e?.ingress_value||"").trim();for(let[i,o]of(e?.stages||[]).entries()){let r=(o?.targets||[]).filter(v=>v?.enabled!==!1),l=t==="sip"&&i===0,x=r.length+(l&&!r.some(v=>String(v?.kind||"").trim().toLowerCase()==="sip"&&String(v?.value||"").trim()===s)?1:0),k=Number(o?.ring_seconds),S=Number(o?.max_call_seconds||0);if(!x)return`Stage ${i+1} needs at least one active destination.`;if(!Number.isFinite(k)||k<3||k>300)return`Stage ${i+1} ring time must be between 3 and 300 seconds.`;if(!Number.isFinite(S)||S<0||S>86400||S>0&&S<10)return`Stage ${i+1} connected-call limit must be 0 (unlimited) or between 10 and 86400 seconds.`;if(o?.answer_mode==="conference"&&r.some(v=>v.kind!=="sip"))return`Stage ${i+1} conference mode supports SIP phones only.`;let C=r.filter(v=>String(v?.kind||"").trim().toLowerCase()==="external");if(C.length){if(x!==1)return`Stage ${i+1} outside forwarding must be the only destination in its stage. This prevents an analog gateway from answering and cancelling parallel phones.`;if(o?.answer_mode!=="first_answer")return`Stage ${i+1} outside forwarding must use First answer wins.`;if(t==="gateway"&&String(C[0]?.trunk||"").trim()===s)return`Stage ${i+1} cannot send the call back out through incoming gateway ${s}. Choose a separate outbound gateway.`}for(let v of r){if(v?.enabled===!1)continue;let y=String(v?.kind||"").trim().toLowerCase(),m=String(v?.value||"").trim(),O=String(v?.trunk||"").trim();if(!m)return`Stage ${i+1} has a destination with no value selected.`;if(y==="external"&&!O)return`Stage ${i+1} outside number ${m} needs an outbound gateway.`;if(t!=="manual"&&(y==="sip"||y==="gateway")&&m===s&&!(t==="sip"&&y==="sip"&&m===s&&i===0))return`Stage ${i+1} routes ${m} back to its own incoming source.`;let I=y==="sip"||y==="gateway"?`endpoint:${m}`:`${y}:${m}:${O}`;if(a.has(I))return`${m} is already used in stage ${a.get(I)}. A destination can appear only once in a route plan.`;a.set(I,i+1)}}return""}async function he(){if(!n.advancedDraft)return;let e=G(n.advancedDraft),a=D(e);if(a){g(a),u();return}let t=e.id?`api/advanced-routes/${encodeURIComponent(e.id)}`:"api/advanced-routes",s=await _(t,{method:e.id?"PUT":"POST",body:JSON.stringify(e)}),i=n.advancedRoutes.findIndex(o=>o.id===s.id);i>=0?n.advancedRoutes[i]=s:n.advancedRoutes.push(s),n.advancedDraft=structuredClone(s),g(e.id?"Route plan updated.":"Route plan created."),u()}async function ye(e){let a=n.advancedRoutes.find(t=>t.id===e);!a||!window.confirm(`Delete route plan \u201C${a.name}\u201D? Existing legacy routes are not affected.`)||(await _(`api/advanced-routes/${encodeURIComponent(e)}`,{method:"DELETE"}),n.advancedRoutes=n.advancedRoutes.filter(t=>t.id!==e),n.advancedDraft?.id===e&&(n.advancedDraft=null),g("Route plan deleted."),u())}async function we(e,a){let t=n.advancedRoutes.find(r=>String(r.id)===String(e));if(!t)return;if(a&&(t.stages||[]).some(r=>r.answer_mode==="private_hub"))throw new Error("Private hub plans require the isolated-media worker and cannot be activated as a shared conference.");let s=G({...t,enabled:a}),i=await _(`api/advanced-routes/${encodeURIComponent(e)}`,{method:"PUT",body:JSON.stringify(s)}),o=n.advancedRoutes.findIndex(r=>String(r.id)===String(e));o>=0&&(n.advancedRoutes[o]=i),n.advancedDraft?.id===e&&(n.advancedDraft=structuredClone(i)),g(a?"Route plan is live.":"Route plan saved as draft."),u()}function $e(){let e=b("quick-kind").value,a=b("quick-label").value.trim(),t=b("quick-value").value.trim();if(!a||!t){g("Route needs a name and destination.");return}let s=E(a||t),i=e==="intercom"?"gateway":e,o={id:s,label:a,type:i,node_id:i==="node"?t:"",extension:i!=="node"?t:"",trunk:i==="gateway"?A():"",timeout:w().routing.ring_seconds||25,fallback_targets:F(b("quick-fallbacks").value)};w().call_targets.push(o),P("Route added. Save to keep it."),u()}function _e(e){w().call_targets.splice(Number(e),1),P("Route deleted. Save to keep it."),u()}function ke(e,a){let t=w();t.route_overrides[e]={mode:a,reason:""},P("Availability changed. Save to keep it."),u()}function M(){let e=f()[0]?.extension||"",a=n.sip.map(p).filter(Boolean).find(t=>!h(t));return{id:"",name:"",ingress_kind:e?"gateway":"sip",ingress_value:e||a?.extension||"",enabled:!0,stages:[{id:`stage_${$(4)}`,name:"First response",ring_seconds:20,answer_mode:"first_answer",max_answered:1,targets:[]}]}}function L(){let e=n.advancedRoutes||[],a=n.advancedDraft;return`
    <section class="advanced-routing" aria-labelledby="advanced-routing-title">
      ...
      <div class="route-plan-list">
        ...
        ${e.map(U).join("")||'<div class="empty compact-empty">No advanced plans. Existing gateway and SIP routing remains unchanged.</div>'}
      </div>
      ${a?B(a):""}
    </section>`}function j(){let e=n.callFeatures||{},a=n.sip.map(p).filter(l=>l&&l.enabled!==!1&&!h(l)),t=n.sip.map(p).filter(l=>l&&l.enabled!==!1&&h(l)),s=n.activeInvite.source_extension||a[0]?.extension||"",i=a.map(l=>`<option value="${d(l.extension)}">${d(l.extension)} \xB7 ${d(l.description||l.username||"SIP phone")}</option>`).join(""),o=a.map(l=>`<option value="${d(l.extension)}">${d(l.description||l.username||l.extension)}</option>`).join(""),r=t.length?`Outside format: *${d(t[0].extension)}*number`:"Add an enabled gateway before inviting an outside number.";return`<div class="site-feature-card">
  ...
  </div>`}function U(e){let a=Array.isArray(e.stages)?e.stages:[],t=f().filter(i=>String(i.gateway_inbound_mode||"")==="direct_target"&&String(i.gateway_direct_target||"")===String(e.id||"")),s=a.map((i,o)=>{let l=(i.targets||[]).filter(k=>k.enabled!==!1).map(V);o===0&&e.ingress_kind==="sip"&&l.unshift(`${q(e)} (primary)`);let x=l.join(" + ")||"no destination";return`<span class="route-stage-pill"><b>${o+1}</b><span>${d(i.name||`Stage ${o+1}`)}<small>${d(x)} \xB7 ${d(i.ring_seconds||0)}s</small></span></span>`}).join('<span class="route-arrow">\u2192</span>');return`<article class="route-plan-summary ${e.enabled?"":"disabled"}">
    <div class="route-plan-main">
      ...
      <div class="route-plan-ingress"><span>${e.ingress_kind==="sip"?"Calls landing on":"Calls entering from"}</span><b>${d(q(e))}</b></div>
  ...
  </article>`}function q(e){let a=String(e?.ingress_value||""),t=n.sip.map(p).filter(Boolean).find(i=>String(i.extension)===a),s=t?.description||t?.username||a||"not selected";return`${e?.ingress_kind==="gateway"?"Gateway":"SIP phone"} ${a}${s&&s!==a?` \xB7 ${s}`:""}`}function V(e){let a=String(e?.value||"");if(e?.kind==="external")return`Gateway ${e.trunk||"not selected"} dials ${a||"no number selected"}`;if(e?.kind==="haos")return`HAOS ${n.nodes.find(o=>String(o.id)===a)?.label||a}`;let t=n.sip.map(p).filter(Boolean).find(i=>String(i.extension)===a);return`${e?.kind==="gateway"?"Gateway/FXO":"SIP"} ${a}${t?.description?` \xB7 ${t.description}`:""}`}function B(e){let a=Array.isArray(e.stages)?e.stages:[],t=a.some(i=>i.answer_mode==="private_hub"),s=D(e);return`<form class="advanced-editor" id="advanced-route-form">
    ...
    <div class="advanced-route-basics">
      ...
      <div class="field"><label>${e.ingress_kind==="sip"?"Called SIP phone":"Incoming gateway"}</label>${J(e)}</div>
  ...
  </form>`}function J(e){let a=e.ingress_kind==="gateway"?f():n.sip.map(p).filter(s=>s&&!h(s));return`<select data-advanced-route-key="ingress_value">
    ...
    <div class="stage-body">
      ...
      <div class="stage-policy">${W(e)}</div>
      ...
      <div class="advanced-targets">
        ${t.map((o,r)=>X(o,a,r,e.answer_mode)).join("")||'<div class="empty compact-empty">Add one or more destinations for this stage.</div>'}
      </div>
      <button type="button" class="text-button" data-action="advanced-add-target" data-stage="${a}">+ Add parallel destination</button>
    </div>
  </article>`}function W(e){let a=e?.answer_mode;if(a==="conference"){let t=(e?.targets||[]).filter(i=>i?.enabled!==!1).length,s=Math.max(1,Number(e?.max_answered)||1);return`All ${t} active SIP destination${t===1?"":"s"} ring together. Up to ${s} answered destination${s===1?"":"s"} stay in the conference; the original caller is additional and is not counted in this limit.`}return a==="private_hub"?"Isolation contract: the hub may talk to every spoke while spokes remain private. This plan stays draft until the ARI isolated-media worker is installed; Simson will never substitute a shared conference that leaks spoke audio.":"The first destination to answer owns the call; all other ringing destinations are cancelled immediately."}function X(e,a,t,s){let i=e.kind==="external"?"Number or intercom extension to dial":e.kind==="haos"?"HAOS node/card":e.kind==="gateway"?"Gateway or wired port":"SIP phone",o=K(e);return`<div class="advanced-target ${e.enabled===!1?"disabled":""}">
    ...
    <div class="advanced-target-field destination"><label>${d(i)}</label>${Q(e,a,t)}</div>
    ...
    <div class="card" style="margin-top:16px">
      ...
      <div class="list" id="target-list">
        ${e.call_targets.map(H).join("")||'<div class="empty">No route targets yet.</div>'}
      </div>
    </div>
    ${L()}
  `}export{oe as a,le as b,ce as c,ue as d,ve as e,pe as f,ge as g,be as h,T as i,me as j,fe as k,he as l,ye as m,we as n,$e as o,_e as p,ke as q,u as r};
