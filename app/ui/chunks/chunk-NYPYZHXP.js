import{c as m,f as O}from"./chunk-RP7N3B43.js";import{a as U,e as s,f as C,h as d}from"./chunk-6R6CGC23.js";import{a as u,b as i,d as c,e as L}from"./chunk-OPL7LVHU.js";function H(){let e=d(),t=String(e.local_api_port||C.local_api_port||8799),a=window.location.hostname||"homeassistant.local";return`${window.location.port===t?window.location.protocol:"http:"}//${a}:${t}`}function k(e="TRIGGER_ID"){return`/api/automation/device/${d().automation.webhook_id||"WEBHOOK_ID"}/${e}`}function x(e="TRIGGER_ID"){return`${H()}${k(e)}`}function S(){return`/api/automation/webhook/${d().automation.webhook_id||"WEBHOOK_ID"}`}function M(){return`${H()}${S()}`}function X(e=24){let t=new Uint8Array(e);if(window.crypto&&typeof window.crypto.getRandomValues=="function")window.crypto.getRandomValues(t);else for(let a=0;a<t.length;a+=1)t[a]=Math.floor(Math.random()*256);return Array.from(t,a=>a.toString(16).padStart(2,"0")).join("")}function Y(){let e=u("door-trigger-select")?.value||"",t=e?(d().automation.triggers||[]).find(n=>n.id===e):null,a=u("door-source")?.value||"",r=s.selectedDoorTriggerId;t&&t.source_extension&&t.source_extension!==a?s.selectedDoorTriggerId="":s.selectedDoorTriggerId=e,s.selectedDoorTriggerId!==r&&N()}function $(e){let t=Number(e),a=Number.isFinite(t)&&t>0?t:90;return Math.max(20,Math.min(3600,Math.round(a)))}function y(e){let t=String(e||"").trim();if(!t)return"No outdoor source selected";let a=s.sip.map(m).find(n=>n&&String(n.extension)===t);return`${a?.description||a?.username||t} (${t})`}function D(e){let t=Array.isArray(e)?e.filter(Boolean):[];return t.length?t.map(_).join(" + "):"No destinations selected"}function N(){let e=d(),t=e.automation,a=$(t.cooldown_seconds),r=s.sip.map(m).filter(o=>o&&o.enabled!==!1&&o.video_enabled),n=e.call_targets.filter(o=>["sip","asterisk","node","device"].includes(o.type)),v=(t.triggers||[]).filter(o=>o.mode==="door_station"),b=s.selectedDoorTriggerId||"",l=v.find(o=>o.id===b)||{},g=Array.isArray(l.target_ids)&&l.target_ids.length?l.target_ids:[l.target_id].filter(Boolean),p=new Set(g.map(String)),h=l.source_extension||r[0]?.extension||"",I=l.fanout_mode||"parallel",f=new Set(L(t.notify_services)),B=new Set(s.notificationTargets.map(o=>String(o.ref||""))),E=new Set(s.notificationTargets.filter(o=>o.rich_actions).map(o=>String(o.ref||""))),T=s.notificationTargets.filter(o=>{if(o.kind!=="entity")return!0;let w=String(o.ref||"").replace(/^notify\./,"");return!E.has(`notify.mobile_app_${w}`)||f.has(String(o.ref))}).filter(o=>o.rich_actions||f.has(String(o.ref))).sort((o,w)=>{let R=Number(f.has(String(w.ref)))-Number(f.has(String(o.ref)));return R||String(o.label||o.ref).localeCompare(String(w.label||w.ref))}),P=T.filter(o=>f.has(String(o.ref))),A=[...f].filter(o=>!B.has(o));u("content").innerHTML=`
    ...
    <div class="card" style="margin-top:16px">
      ...
      <div class="form-grid">
        ...
        <div class="field full">
          ...
          <input id="intercom-url" class="mono" readonly value="${i(j())}">
    ...
    <div class="card" style="margin-top:16px">
      ...
      <div class="list">
        ${t.triggers.map(z).join("")||'<div class="empty">No automation triggers yet.</div>'}
      </div>
    </div>
  `}function G(){let e=String(s.status?.server_url||U.server_url||"").trim();return e?e.startsWith("wss://")?`https://${e.slice(6).replace(/\/ws\/?$/,"")}`:e.startsWith("ws://")?`http://${e.slice(5).replace(/\/ws\/?$/,"")}`:e.replace(/\/ws\/?$/,"").replace(/\/$/,""):""}function j(){let e=u("intercom-source")?.value||s.sip.map(m).filter(Boolean)[0]?.extension||"",t=d().call_targets[0]?.id||s.sip.map(m).filter(Boolean).find(h=>h.extension!==e)?.extension||"",a=u("intercom-target")?.value||t,r=u("intercom-source-mode")?.value||"speaker",n=u("intercom-target-mode")?.value||"speaker",v=G(),b=s.status?.account_id||"",l=s.status?.node_id||"",g=s.status?.install_token||"";if(!v||!b||!l||!g||!e||!a)return"";let p=new URL("/node/sip-intercom",v);return p.searchParams.set("account_id",b),p.searchParams.set("node_id",l),p.searchParams.set("install_token",g),p.searchParams.set("source",e),p.searchParams.set("target",a),p.searchParams.set("source_auto_mode",r),p.searchParams.set("target_auto_mode",n),p.searchParams.set("timeout_sec","30"),p.toString()}function se(){let e=u("intercom-url");e&&(e.value=j())}function F(e){return e?["sip","asterisk"].includes(e.type)?`SIP/video extension ${e.extension||e.id}`:["node","device"].includes(e.type)?`HAOS node ${e.node_id||e.id}`:`${e.type||"target"} ${e.extension||e.node_id||e.id}`:""}function _(e){let t=d().call_targets.find(n=>String(n.id)===String(e));if(!t)return String(e||"");let a=t.label||t.id,r=t.extension?` (${t.extension})`:t.node_id?` (${t.node_id})`:"";return`${a}${r}`}function W(e){if(!e.webhook_id)return'<div class="empty" style="margin-top:14px">Generate credentials to get device callback URLs.</div>';let t=(d().automation.triggers||[]).filter(a=>a.mode==="door_station"&&String(a.id||"").trim());return t.length===1?`
      <div class="row" style="margin-top:14px">
        ...
        <input class="mono" readonly value="${i(M())}" style="margin-top:8px;">
    ...
    `).join("")}
  `}function z(e){let t=Array.isArray(e.target_ids)&&e.target_ids.length?e.target_ids:[e.target_id].filter(Boolean),a=t.map(_).join(" + "),r=e.mode==="door_station"?$(e.cooldown_seconds||d().automation.cooldown_seconds):e.cooldown_seconds||d().automation.cooldown_seconds||90,n=e.mode==="door_station",v=e.fanout_mode==="priority"?"priority order":"same time";return`
  ...
  `}export{N as a,G as b,j as c,se as d,F as e,_ as f,W as g,z as h,X as i,Y as j,$ as k,y as l};
